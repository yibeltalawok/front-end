            ...
            <v-card-text>
              <v-form @submit.prevent="save">
                ...
                <v-layout row wrap justify-center>
                  <v-flex xs12 sm12 md10 lg10 pa-2>
                    <v-radio-group v-model="item.type" :mandatory="false">
                      <v-layout row wrap>
                        <v-flex>
...
export default {
  ...
  async created() {
    const { overtimeId } = this.$route.params;
    this.item = await api.get(overtimeId, path.overtime);
    this.payrollId = this.item.payrollId;
    this.camps = [
      {
        text: "Dashboard",
        disabled: false,
      ...
      },
      {
        ...
        disabled: false,
        to: "/app/payroll/master/" + this.payrollId + "/info"
      },
      {
        text: "Over Time",
        disabled: false,
        to: "/app/over-time/" + this.payrollId + "/list"
      },
      {
  ...
  methods: {
    save() {
      api
        .update(this.item, path.overtime)
        .then(() => {
          this.item = {};
          this.$notify({
            type: "success",
