...
export default {
  ...
  methods: {
    ...
    async fetchBundle() {
      // console.log("fetchBundle Called")
      // eslint-disable-next-line no-unused-vars
      await this.fetchBundleData(this.orderId).then((res) => {
        this.bndlnum = this.bundles.bndlnum;
      });
    ...
    async drawGraph(i) {
      ...
      table.rows[rowInserted].cells[0].rowSpan =
        this.bundlesInOrder[i].length - 1;
    ...
    async stepUpTo(id, status) {
      ...
      await this.updateBundleStatus([id, status]).then(() => {
        ...
        // alert(this.updatedStatus)
      });
    },
    async stepDownTo(id, status) {
      await this.updateBundleStatus([id, status]);
      this.fetchBundle();
      this.initiateSingleOrderStatus(this.orderId);
    },
