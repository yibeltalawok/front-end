        ...
        <v-card-text>
          <v-form @submit.prevent="save">
            <v-layout row wrap justify-center my-3>
              ...
              <v-flex xs12 sm12 md4 lg4 pa-2>
                <v-text-field
                  :label="$t('actual-sewing-end-date')"
                  placeholder="yyyy-mm-dd"
                  dense
                  outlined
...
export default {
  ...
  methods: {
    ...
    save() {
      api
        .create(this.item, path.timeAction)
        .then(() => {
          this.$notify({
            type: "success",
            title: "Time and Action",
            message: `Time and Action Calendar information successfully saved.`
          });
