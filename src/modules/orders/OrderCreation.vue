                ...
                <tr>
                  <td>
                    <v-text-field
                      ...
                      name="orderNumber"
                      @input="checkOrderNumber(item.orderNumber)"
                    />
                  </td>
                  <!---->
                  <td>
...
export default {
  ...
  methods: {
    ...
    ...mapActions("order", ["getOrderNumbers"]),
    
    test() {
      for (let i = 0; i < this.tempValue.length; i++) {
        this.tempValue[i] = parseInt(this.tempValue[i]);
      }
   ...
   //select product name using productId 
     ...
     checkOrderNumber(ordernum){
      ...
      if(this.orderNumbers.includes(ordernum)){ // cutNumbers array contains cutnum
        this.errormsg.push("Order No. " + ordernum + " already exists")
      }
     },
    ...
    async onFileSelected() {
      ...
      // this.selectedFileImageUrl = URL.createObjectURL(
      //   this.selectedFile
      // );
    ...
    async uploadFile(file){
      ...
      this.item.attachment = this.fileName;
      formData.append("File", file, this.fileName);
      await api.upload("orders", formData, this.pathContainer);
    },
    ...
    async save() {
      ...
      if(this.errormsg.length == 0 ){
        await this.computeAndSave(this.tempValue).then(ret => {
          if (ret) {
            this.$validator.validateAll().then(valid => {
              if (valid) {
                ...
                this.postData([this.item, path.order])
                  .then(() => {
                    this.$notify({
                      type: "success",
                      title: "Order",
