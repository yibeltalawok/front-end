    ...
    <v-container grid-list-xs>
      <v-card outlined style="border-radius: 12px;">
        ...
        <v-form @submit.prevent="save" ref="form" lazy-validation>
          <v-layout row>
            <v-flex class="ml-4 mt-4" xs12 sm12 md12 lg12 pa-1>
              <v-layout row>
            ...
            <v-flex class="ml-4 mt-1" xs12 sm12 md12 lg12 pa-1>
              <v-layout row>
                <v-flex xs12 sm12 md3 lg3 pa-1>
                  <v-text-field
                    dense
            ...
            <v-flex class="ml-4 mt-1" xs12 sm12 md12 lg12 pa-1>
              <v-layout row>
                <v-flex class="mr-1" xs12 sm12 md3 lg3>
                  <v-text-field
                    dense
                ...
                <v-flex class="mr-1" xs12 sm12 md2 lg2 pa-1>
                  <v-autocomplete
                    :items="category"
                    dense
                    hide-details
          ...
          <v-layout
            row
            justify-center
            style="margin-bottom: 20px; margin-left: 20%;"
          >
            <v-flex xs12 sm6>
              <v-btn outlined color="amber" @click="save">
                {{ $t('save') }}
              </v-btn>
            </v-flex>
          </v-layout>
...
export default {
  ...
  methods: {
    ...mapActions('inventory', ['postData', 'updateProduct']),
    ...
    async save() {
      ...
      if (this.$refs.form.validate()) {
        ...
        this.updateProduct([this.item]).then(() => {
          this.$router.push({ name: 'product-list' })
          this.$notify({
            type: 'success',
            title: this.$t('update product'),
...
.imagePreviewProduct h4 {
  font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif;
  color: green;
  text-decoration: underline;
}
.imagePreviewProduct img {
  width: 100%;
  height: 120px;
}
#rounded-card {
