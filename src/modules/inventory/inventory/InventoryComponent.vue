            ...
            <v-layout row justify-center>
                ...
                <v-layout column justify-center>
                  ...
                  <v-flex xs12 sm12 md12 lg12>
                    <v-text-field
                      ...
                      outlined
                      :rules="inputRule"
                      @input="checkInventoryNames(item.inventoryName)"
                    ></v-text-field>
                  </v-flex>
                </v-layout>
              </v-flex>
              ...
              <v-flex xs12 sm12 md4 lg4 pr-5>
                <v-layout column justify-center>
                  ...
                  <v-flex xs12 sm12 md12 lg12>
                    <!-- TODO must display the category name ... -->
                    <v-combobox
                      flat
                      dense
                      hide-details
              ...
              <v-flex xs12 sm12 md4 lg4>
                <v-layout column justify-center>
                  ...
                  <v-layout row justify-center pl-5>
                    <span style="width: 50%;">
                      <input
                        type="checkbox"
                        id="sellable"
                        @change="item.sellable = !item.sellable"
                      />
                    ...
                    <span style="width: 50%;">
                      <input
                        type="checkbox"
                        id="obsolete"
                        @change="item.obsolete = !item.obsolete"
                      />
                    ...
                    <span style="width: 50%;">
                      <input
                        type="checkbox"
                        id="neverDiminshing"
                        @change="item.neverDiminshing = !item.neverDiminshing"
                      />
                      <label for="neverDiminshing">
                        Never diminshing
                      </label>
                    </span>
                    <span style="width: 50%;">
                      <input
                        type="checkbox"
                        id="component"
                        @change="item.component = !item.component"
                      />
                    ...
                    <span style="width: 50%;">
                      <input
                        type="checkbox"
                        id="assempledProduct"
                        @change="item.assempledProduct = !item.assempledProduct"
                      />
                      <label for="assempledProduct">
                        Assempled product
                      </label>
                    </span>
                    <span style="width: 50%;">
                      <!-- <input
                              type="checkbox"
                              id="expirable"
                              @change="changeExpiryState"
                            />
                            <label for="expirable"> Expirable</label> -->
                    </span>
                  </v-layout>
                </v-layout>
              </v-flex>
    ...
    <v-row justify="center">
      <v-dialog v-model="dialog" persistent max-width="400px">
        <v-card wrap outlined style="border-radius: 1px;">
          ...
          <CategoryPopup :item="this.catItem" />
...
export default {
  ...
  methods: {
    ...
    mounted() {
      ...
      // this.categoryName = this.category.categoryName; // this is to bind the category name...
    ...
    checkInventoryNames(invname) {
      this.errormsg = []
      let invNm = invname.toLowerCase()
      let inventoryNm = []
      for (let i = 0; i < this.inventoryNames.length; i++) {
        inventoryNm.push(this.inventoryNames[i].toLowerCase())
        if (i == this.inventoryNames.length - 1) {
          if (inventoryNm.includes(invNm)) {
            this.errormsg.push('Inventory Name ' + invname + ' already exists')
          }
        }
      }
