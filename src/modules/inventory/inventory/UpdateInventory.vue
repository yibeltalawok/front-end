    ...
    <v-container grid-list-xs>
      <v-card outlined style="border-radius: 12px;">
        <v-toolbar dense flat>
          ...
          <v-spacer />
          <h2 class="mt-3">Update Inventory</h2>
          <v-spacer />
          <v-btn icon>
        ...
        <v-card-text>
          ...
          <InventoryComponent :item="this.itemById" />
...
export default {
  ...
  async mounted() {
    // this.image = this.image = API_ROOT + '/Containers/inventory/download/' + itemById.imagePath
    const { inventoryId } = this.$route.params
    this.inventoryId = inventoryId
    let filter = { include: ['category'] }

    this.getDataByIdWithF([inventoryId, path.inventory, filter]).then((res) => {
      res
      this.categoryName = this.itemById.category.categoryName
      this.itemById.createdById = this.user.id
      this.getProfileImage(['inventory/download', this.itemById.imagePath])
    })
  ...
  methods: {
    ...
    async save() {
      ...
      this.updateData([this.itemById, path.inventory])
        .then(() => {
          this.$router.push({ name: 'inventory' })
          this.$notify({
            type: 'success',
            title: this.$t('update inventory'),
    ...
    async getCategoryId(val) {
      this.itemById.categoryId = val.id
    },
    async onInventoryFileSelected(event) {
    ...
    async uploadPicture(file) {
      ...
      let name = this.itemById.inventoryName
      this.itemById.imagePath = name
...
<style scoped>
.imagePreviewInventory {
  text-align: center;
  margin-top: -40px;
}
.imagePreviewInventory h4 {
  font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif;
  color: green;
  text-decoration: underline;
}
.imagePreviewInventory img {
  width: 100%;
  height: 120px;
}
